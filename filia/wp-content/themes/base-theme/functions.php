...
//SUBIR CV
if(isset($_POST['action']) AND $_POST['action'] == 'registroformulario' ) wp_subir_cv();
...
// FRONT END SCRIPTS AND STYLES //////////////////////////////////////////////////////
	...
	function wp_subir_cv() {

		if (isset($_POST['post_id']) && is_numeric($_POST['post_id']) && isset($_FILES['uploadedFile']) && $_FILES['uploadedFile']['error'] === UPLOAD_ERR_OK) {
			...
			if ($fileSize > 5000000) {
				$_SESSION['error'] = "El tipo archivo es muy grande!";
				header("Location: ".home_url('vacantes'));
				die();
			}

			$allowedfileExtensions = array('doc', 'pdf', 'docx');
			if (in_array($fileExtension, $allowedfileExtensions)) {
				$uploadFileDir = get_template_directory();
				$dest_path = $uploadFileDir ."/candidatos/". $post ."/".$newFileName;

				if ( !file_exists($uploadFileDir ."/candidatos/".$post) ) {
					if (!mkdir($uploadFileDir ."/candidatos/".$post, 0777, true)) {
						$_SESSION['error'] = "No pudimos mandar tu informacion, intentalo más tarde!";
						header("Location: ".home_url('vacantes'));
						die();
					}
				}
			...
			$_SESSION['error'] = "El tipo de archivo no es valido!";
			header("Location: ".home_url('vacantes'));
			die();
		}
		$_SESSION['error'] = "No pudimos mandar tu informacion, intentalo más tarde!";
